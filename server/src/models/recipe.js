  ...
  if (recipe.videoUrl) {
    if (!recipe.videoUrl.includes("https://www.youtube.com/embed/")) {
      ...
      if (recipe.videoUrl.includes("&")) {
        const videoId = recipe.videoUrl.slice(
          recipe.videoUrl.lastIndexOf("v=") + 2,
          recipe.videoUrl.indexOf("&")
        );
  ...
  if (!recipe.steps || recipe.steps.length == 0) {
    ...
    recipe.steps.forEach((step, index) => {
      if (step.trim().length < 4) {
        throw new Error(
          `Step number ${index + 1} must contain at least 5 characters.`
        );
      }
    });
  }
...
recipeSchema.statics.saveRecipe = async (recipeReq) => {
  ...
  const ingredientsDb = await Ingredient.saveIngredients(
...
recipeSchema.statics.orderByBestRated = (
  recipes,
  limit = null,
  skip = null
) => {
...
recipeSchema.statics.orderByLessIngredients = (
  recipes,
  limit = null,
  skip = null
) => {
  ...
  recipes.sort((recipeA, recipeB) =>
    recipeA.ingredients.length <= recipeB.ingredients.length ? -1 : 1
  );
...
recipeSchema.statics.orderByLessMaterials = (
  recipes,
  limit = null,
  skip = null
) => {
  ...
  recipes.sort((recipeA, recipeB) => {
    if (
      recipeA.materials &&
      recipeA.materials.length <= recipeB.materials &&
      recipeB.materials.length
    ) {
